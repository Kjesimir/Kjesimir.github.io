...
if ( ! function_exists( 'startbootstrap_setup' ) ) :
	...
	function startbootstrap_setup() {
		 ...
		 * If you're building a theme based on StartBootstrap, use a find and replace
		 * to change 'startbootstrap' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'startbootstrap', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'startbootstrap' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'startbootstrap_custom_background_args',
				array(
 ...
 */
function startbootstrap_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'startbootstrap_content_width', 640 );
}
add_action( 'after_setup_theme', 'startbootstrap_content_width', 0 );
...
function startbootstrap_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'startbootstrap' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'startbootstrap' ),
